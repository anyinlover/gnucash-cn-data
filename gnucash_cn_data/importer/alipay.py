...
class AliPay(Base):
    def read_csv(self):
        ...
        df = pd.read_csv(self.csv_path, encoding="gb18030", skiprows=24, index_col=False)
    ...
    def fix_method(self):
        def fix_func(row: pd.Series) -> pd.Series:
            """Fix method"""
            if row["收/付款方式"] == "":
                if row["收/支"] == "收入" or re.search("基础软件服务费", row["商品说明"]):
                    row["收/付款方式"] = "账户余额"
                elif row["商品说明"] == "退款-亲情卡":
                    order_id = row["交易订单号"].split("_")[0]
                    row["收/付款方式"] = self.df.loc[self.df["交易订单号"] == order_id, "收/付款方式"].iloc[0]
                else:
                ...
                if not exist_methods:
                    ...
                    raise ValueError(f"复杂模式的收/付款方式{row['收/付款方式']}暂不支持")
                else:
    ...
    def create_format_df(self):
        df = pd.DataFrame()
        df["description"] = self.df["交易分类"] + " " + self.df["交易对方"] + " " + self.df["商品说明"]
